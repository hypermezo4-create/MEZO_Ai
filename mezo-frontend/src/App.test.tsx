...
it("opens directly into the MEZO workspace with no login", async () => {
  ...
  expect(await screen.findByRole("heading", { level: 1, name: "What should MEZO build?" })).toBeTruthy()
...
it("reveals a repository form from the projects section", async () => {
  ...
  await screen.findByRole("heading", { level: 1, name: "What should MEZO build?" })
