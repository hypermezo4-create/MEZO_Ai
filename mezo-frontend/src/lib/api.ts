...
export interface Task { id: string; conversation_id: string; project_id: string; prompt: string; mode: Mode; status: string; changed_files: ChangedFile[]; diff_text: string; error: string | null; decision: string | null; reviewer_chain: string[]; created_at: string; runner_id: string | null }
export interface DispatchResult { kind: "chat" | "agent"; interaction: "chat" | "agent"; conversation_id: string; message: Message; task: Task | null }
...
export interface ClusterStatus { api: string; database: string; valkey: string; github_configured: boolean; machines: Machine[]; router: { healthy: boolean; models?: Record<string, { healthy: boolean; latency_ms?: number }> } }
...
class Client {
  ...
  dispatch = (input: { prompt: string; conversation_id?: string; project_id?: string; interaction?: Interaction; mode: Mode }) => this.request<DispatchResult>("/api/dispatch", { method: "POST", body: JSON.stringify(input) })
  ...
  stream(id: string, after: number, signal: AbortSignal, onEvent: (event: TaskEvent) => void) {
    return fetch(`/api/tasks/${id}/events?after=${after}`, { signal }).then(async response => {
      ...
      while (true) { const { value, done } = await reader.read(); buffer += decoder.decode(value, { stream: !done }); for (const frame of buffer.split(/\r?\n\r?\n/).slice(0,-1)) { const line = frame.split(/\r?\n/).find(item => item.startsWith("data: ")); if (line) onEvent(JSON.parse(line.slice(6)) as TaskEvent) } if (done) return }
    })
  }
}
